...
import {setRefreshTokenCookie, setAccessTokenCookie} from "../utils/helper-functions";
...
export class AuthController {
    static parentRegister = asyncWrapper(async (req: Request, res: Response) => {
        ...
        await parentService.registerParent(data.firstName, data.lastName, data.email, data.password);
    ...
    static parentLogin = asyncWrapper(async (req: Request, res: Response) => {
        const parentService = Container.get(ParentService);
        const {parent, accessToken, refreshToken} = await parentService.loginParent(req.body.email, req.body.password);
    ...
    static refreshToken = asyncWrapper(async (req: Request, res: Response) => {
        ...
        try {
            ...
            throw new CustomError('Invalid or expired refresh token', 403, 'INVALID_REFRESH_TOKEN');
        }
    });
